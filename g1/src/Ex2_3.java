...
public class Ex2_3 {
    ...
    private static class DirectAccess implements Runnable{

        private final int to;

        private DirectAccess(int to){
            this.to = to;
        }
    ...
    public static void main(String[] args) throws InterruptedException{
        ...
        // Counting with direct access
        for(int j = 0; j < n; j++){
            threadList.push(new Thread(new DirectAccess(i)));
            threadList.peek().start();
        }
        while(!threadList.empty())
            threadList.pop().join();
        System.out.println("Direct access: " + counter.i);
